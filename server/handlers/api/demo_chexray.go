...
// ChexrayRunCam .
func ChexrayRunCam(c *gin.Context) {
	...
	ctx := context.Background()
	// Run cam
	resp, err := capsul.RunChexrayCAM(ctx, &camData)
	if err != nil {
		c.JSON(200, jsonutil.Fail(err))
		return
	}
...
// ChexrayProcess .
func ChexrayProcess(c *gin.Context) {
	// Parse body data
	...
	if infData.File != "" {
		infData.File = staticPath("chexray", procData.Target)
	}
...

	{
		...
		go func() {
			// run inference
			infresult, inferr := capsul.RunChexrayInference(ctx, &infData)
			// copy result
			infResult = infresult
			infError = inferr
			wg.Done()
		...
		go func() {
			// run cam
			camresult, camerr := capsul.RunChexrayCAM(ctx, &camData)
			// copy result
			camResult = camresult
			camError = camerr
			wg.Done()
